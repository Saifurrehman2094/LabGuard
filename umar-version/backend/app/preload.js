...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getAllExams: () => ipcRenderer.invoke('exam:getAll'),
  ...
  // New exam notification listener
  onNewExamCreated: (callback) => {
    const handler = (event, data) => callback(data);
    ipcRenderer.on('exam:new-exam-created', handler);
    
    return () => {
      ipcRenderer.removeListener('exam:new-exam-created', handler);
    };
  },

  // Submission file methods
  uploadSubmissionFile: (examId, filePath, originalName) => 
    ipcRenderer.invoke('submission:upload-file', examId, filePath, originalName),
  getMySubmissionFiles: (examId) => ipcRenderer.invoke('submission:get-my-files', examId),
  getAllSubmissionFiles: (examId) => ipcRenderer.invoke('submission:get-all-files', examId),
  getSubmissionStats: (examId) => ipcRenderer.invoke('submission:get-stats', examId),
  deleteSubmissionFile: (examId, studentId, fileName) => 
    ipcRenderer.invoke('submission:delete-file', examId, studentId, fileName),
  getExamSubmissions: (examId) => ipcRenderer.invoke('submission:get-exam-submissions', examId),
  openFile: (filePath) => ipcRenderer.invoke('file:open', filePath),

  // New submission notification listener
  onNewSubmission: (callback) => {
    const handler = (event, data) => callback(data);
    ipcRenderer.on('submission:new-submission', handler);
    
    return () => {
      ipcRenderer.removeListener('submission:new-submission', handler);
    };
  },

  // Network configuration methods
  getNetworkInfo: () => ipcRenderer.invoke('network:get-info'),
  getNetworkMode: () => ipcRenderer.invoke('network:get-mode'),
  setNetworkMode: (mode) => ipcRenderer.invoke('network:set-mode', mode),
  setNetworkServer: (serverHost) => ipcRenderer.invoke('network:set-server', serverHost),
  setNetworkDatabasePath: (databasePath) => ipcRenderer.invoke('network:set-database-path', databasePath),
  getNetworkDatabasePath: () => ipcRenderer.invoke('network:get-database-path')
});
