...
ipcMain.handle('db:getAvailableExams', async (event, studentId) => {
  try {
    ...
    const exams = await dbService.getAvailableExams(studentId);
    return {
      success: true,
      exams
    };
...
ipcMain.handle('db:getStudentExamHistory', async (event, studentId) => {
  try {
    ...
    const history = await dbService.getStudentExamHistory(studentId);
    return {
      success: true,
      exams: history
    };
...
ipcMain.handle('monitoring:get-violations', async (event, examId) => {
  try {
    ...
    // Get violation data
    const violations = await dbService.getAppViolationsByExam(examId);
    const stats = await dbService.getViolationStatsByExam(examId);
    const participantCount = await dbService.getExamParticipantsCount(examId);
...
ipcMain.handle('monitoring:get-student-violations', async (event, examId) => {
  try {
    ...
    const violations = examId
      ? await dbService.getAppViolationsByStudent(currentUser.userId, examId)
      : await dbService.getAllViolationsByStudent(currentUser.userId);
  ...
  try {
    ...
    if (pdfPath && examData.pdfFileName) {
      try {
        ...
        await dbService.updateExam(exam.examId, { pdfPath: newUploadResult.filePath });
    ...
    const course = await dbService.getCourseById(examData.courseId);
    
    // Get all enrolled students
    const enrolledStudents = await dbService.getEnrolledStudents(examData.courseId);
    
    // Send real-time notification to all enrolled students
    if (mainWindow && mainWindow.webContents && enrolledStudents.length > 0) {
      mainWindow.webContents.send('exam:new-exam-created', {
        exam: exam,
      ...
      console.log(`Notified ${enrolledStudents.length} students about new exam: ${exam.title}`);
    }
...
ipcMain.handle('exam:getByTeacher', async (event, teacherId) => {
  try {
    ...
    const exams = await dbService.getExamsByTeacher(teacherId);
    return {
      success: true,
      exams
    };
...
ipcMain.handle('exam:getAll', async (event) => {
  try {
    ...
    const exams = await dbService.getAllExams();
    return {
      success: true,
      exams
    };
...
ipcMain.handle('exam:update', async (event, updateData) => {
  try {
    ...
    const existingExam = await dbService.getExamById(updateData.examId);
    if (!existingExam) {
      return {
        success: false,
        error: 'Exam not found'
    ...
    const success = await dbService.updateExam(updateData.examId, updateFields);
    ...
    // Return updated exam
    const updatedExam = await dbService.getExamById(updateData.examId);
    return {
      success: true,
      exam: updatedExam
    };
...
ipcMain.handle('exam:delete', async (event, examId) => {
  try {
    ...
    // Get existing exam to verify ownership
    const existingExam = await dbService.getExamById(examId);
    if (!existingExam) {
      return {
        success: false,
        error: 'Exam not found'
    ...
    const success = await dbService.deleteExam(examId);
...
ipcMain.handle('exam:submit', async (event, examId, filesData) => {
  try {
    ...
    const submission = await dbService.submitExam(examId, currentUser.userId, filesData);

    // Get exam details for notification
    const exam = await dbService.getExamById(examId);
    
    // Send real-time notification to teacher
    if (mainWindow && mainWindow.webContents && exam) {
      ...
      console.log(`Notified teacher about submission from ${currentUser.fullName} for exam: ${exam.title}`);
    }
...
ipcMain.handle('exam:get-submission', async (event, examId) => {
  try {
    ...
    const submission = await dbService.getExamSubmission(examId, currentUser.userId);
...
ipcMain.handle('exam:unsubmit', async (event, examId) => {
  try {
    ...
    await dbService.unsubmitExam(examId, currentUser.userId);
...
ipcMain.handle('submission:upload-file', async (event, examId, filePath, originalName) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || currentUser.role !== 'student') {
      return {
        success: false,
        error: 'Unauthorized: Only students can upload submissions'
      };
    }

    const result = await fileService.uploadSubmission(filePath, examId, currentUser.userId, originalName);
...
ipcMain.handle('submission:get-my-files', async (event, examId) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || currentUser.role !== 'student') {
      return {
    ...
    const files = fileService.getStudentSubmissions(examId, currentUser.userId);
...
ipcMain.handle('submission:get-all-files', async (event, examId) => {
  try {
    console.log('[IPC] submission:get-all-files called for exam:', examId);
    
    const currentUser = authService.getCurrentUser();
    console.log('[IPC] Current user:', currentUser?.username, 'Role:', currentUser?.role);
    
    if (!currentUser || (currentUser.role !== 'teacher' && currentUser.role !== 'admin')) {
      console.log('[IPC] Authorization failed - not teacher/admin');
      return {
        success: false,
        error: 'Unauthorized: Only teachers and admins can view all submissions'
      };
    }

    // For teachers, verify they own the exam (admins can view any exam)
    if (currentUser.role === 'teacher') {
      const exam = await dbService.getExamById(examId);
      console.log('[IPC] Exam owner check - Exam teacherId:', exam?.teacherId, 'Current userId:', currentUser.userId);
      
      if (!exam || exam.teacherId !== currentUser.userId) {
        console.log('[IPC] Authorization failed - not exam owner');
        return {
          success: false,
          error: 'Unauthorized: Cannot view submissions for other teacher\'s exams'
        };
      }
    }

    const files = fileService.getAllExamSubmissions(examId);
    console.log('[IPC] Found', files.length, 'submission files');
    console.log('[IPC] Files:', JSON.stringify(files, null, 2));
    ...
    console.error('[IPC] Error getting all submissions:', error);
    return {
      success: false,
      error: error.message
    };
...
ipcMain.handle('submission:get-stats', async (event, examId) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || (currentUser.role !== 'teacher' && currentUser.role !== 'admin')) {
      return {
    ...
    const stats = fileService.getSubmissionStats(examId);
...
ipcMain.handle('submission:delete-file', async (event, examId, studentId, fileName) => {
  try {
    const currentUser = authService.getCurrentUser();
    
    // Students can delete their own files, teachers/admins can delete any
    if (currentUser.role === 'student' && currentUser.userId !== studentId) {
      return {
        success: false,
        error: 'Unauthorized: Can only delete your own files'
      };
    }

    const result = fileService.deleteSubmission(examId, studentId, fileName);
...
// Get exam submissions from database (teacher/admin)
ipcMain.handle('submission:get-exam-submissions', async (event, examId) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || (currentUser.role !== 'teacher' && currentUser.role !== 'admin')) {
      return {
    ...
    const submissions = await dbService.getExamSubmissionRecords(examId);
...
ipcMain.handle('file:open', async (event, filePath) => {
  try {
    ...
    const currentUser = authService.getCurrentUser();
    const accessCheck = fileService.checkFileAccess(filePath, currentUser.userId, currentUser.role);
...
ipcMain.handle('storage:get-info', async (event) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || currentUser.role !== 'admin') {
      return {
    ...
    const info = fileService.getStorageInfo();
...
ipcMain.handle('course:getByTeacher', async (event, teacherId) => {
  try {
    const courses = await dbService.getCoursesByTeacher(teacherId);
...
ipcMain.handle('course:enroll', async (event, courseId, studentId) => {
  try {
    ...
    const enrollment = await dbService.enrollStudent(courseId, studentId);
...
ipcMain.handle('course:getEnrolled', async (event, courseId) => {
  try {
    const students = await dbService.getEnrolledStudents(courseId);
...
ipcMain.handle('course:getStudentCourses', async (event, studentId) => {
  try {
    const courses = await dbService.getStudentCourses(studentId);
    return { success: true, courses };
  } catch (error) {
...
ipcMain.handle('course:unenroll', async (event, courseId, studentId) => {
  try {
    ...
    const success = await dbService.unenrollStudent(courseId, studentId);
    return { success };
  } catch (error) {
...
ipcMain.handle('course:selfEnroll', async (event, courseId) => {
  try {
    ...
    const enrollment = await dbService.enrollStudent(courseId, currentUser.userId);
    return { success: true, enrollment };
  } catch (error) {
...
ipcMain.handle('admin:create-user', async (event, userData) => {
  try {
    ...
    // Log admin action
    await dbService.logAuditEvent(currentUser.userId, 'USER_CREATED', {
...
ipcMain.handle('admin:delete-user', async (event, userId) => {
  try {
    ...
    const userToDelete = await dbService.getUserById(userId);
...
ipcMain.handle('admin:get-audit-logs', async (event, filters) => {
  try {
    ...
    const logs = await dbService.getAuditLogs(filters || {});
...
ipcMain.handle('admin:get-system-settings', async (event) => {
  try {
    ...
    const settings = {
      face_matching_threshold: await dbService.getSystemSetting('face_matching_threshold') || 0.45,
      max_login_attempts: await dbService.getSystemSetting('max_login_attempts') || 5,
      session_timeout: await dbService.getSystemSetting('session_timeout') || 28800000
    };
...
ipcMain.handle('admin:update-system-settings', async (event, settings) => {
  try {
    ...
    for (const [key, value] of Object.entries(settings)) {
      ...
      await dbService.setSystemSetting(key, value, type, null, currentUser.userId);
    }
...
ipcMain.handle('system:get-setup-status', async (event) => {
  try {
    const setupStatus = await dbService.isSystemSetup();
    return {
      success: true,
      data: setupStatus
    };
  ...
  try {
    ...
    const defaultOptions = {
      ...
      filters: [
        ...
        { name: 'Documents', extensions: ['pdf', 'doc', 'docx', 'txt', 'xlsx', 'xls', 'ppt', 'pptx'] },
        { name: 'Images', extensions: ['jpg', 'jpeg', 'png', 'gif', 'bmp'] },
        { name: 'Archives', extensions: ['zip', 'rar', '7z'] },
        { name: 'Code Files', extensions: ['js', 'py', 'java', 'cpp', 'c', 'html', 'css'] }
      ],
...
// Ignore certificate errors for development (suppress SSL warnings)
...
app.on('web-contents-created', (event, contents) => {
  ...
  // Suppress certificate error dialogs
...
const NetworkConfigService = require('../services/networkConfig');
const networkConfigService = new NetworkConfigService();

// Get network information
ipcMain.handle('network:get-info', async (event) => {
  try {
    const networkInfo = networkConfigService.getNetworkInfo();
    return {
      success: true,
      data: networkInfo
    };
...
// Get deployment mode
ipcMain.handle('network:get-mode', async (event) => {
  try {
    const mode = networkConfigService.getDeploymentMode();
    return {
      success: true,
      mode
    };
...
ipcMain.handle('network:set-mode', async (event, mode) => {
  try {
    ...
    const success = networkConfigService.setDeploymentMode(mode);
    
    if (success) {
      // Log admin action
      dbService.logAuditEvent(currentUser.userId, 'NETWORK_MODE_CHANGED', {
        newMode: mode
      });
    }
    ...
    return {
      success,
      mode: networkConfigService.getDeploymentMode()
    };
  } catch (error) {
...
// Set server host
ipcMain.handle('network:set-server', async (event, serverHost) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || currentUser.role !== 'admin') {
      return {
    ...
    const success = networkConfigService.setServerHost(serverHost);
    
    if (success) {
      // Log admin action
      dbService.logAuditEvent(currentUser.userId, 'NETWORK_SERVER_CHANGED', {
        newServer: serverHost
      });
    }

    return {
      success,
      serverHost: networkConfigService.getServerHost()
    };
  } catch (error) {
...
ipcMain.handle('network:set-database-path', async (event, databasePath) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || currentUser.role !== 'admin') {
      return {
    ...
    const success = networkConfigService.setSharedDatabasePath(databasePath);
    
    if (success) {
      // Log admin action
      dbService.logAuditEvent(currentUser.userId, 'NETWORK_DATABASE_PATH_CHANGED', {
...
ipcMain.handle('network:get-database-path', async (event) => {
  try {
    return {
      success: true,
...
ipcMain.handle('network:set-storage-path', async (event, storagePath) => {
  try {
    const currentUser = authService.getCurrentUser();
    if (!currentUser || currentUser.role !== 'admin') {
      return {
    ...
    const success = networkConfigService.setSharedStoragePath(storagePath);
    
    if (success) {
      // Log admin action
      dbService.logAuditEvent(currentUser.userId, 'NETWORK_STORAGE_PATH_CHANGED', {
        newPath: storagePath
      });
    }
...
ipcMain.handle('network:get-storage-paths', async (event) => {
  try {
