...
interface Exam {
  ...
  course_code?: string;
  created_at: string;
  isNew?: boolean; // New exam indicator
}
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, onLogout }) => {
  ...
  const [newExamNotification, setNewExamNotification] = useState<string | null>(null);
  ...
  // Listen for real-time exam creation notifications
  useEffect(() => {
    if (!isElectron()) return;

    const handleNewExam = (data: any) => {
      // Check if this student is enrolled in the course
      if (data.enrolledStudentIds && data.enrolledStudentIds.includes(user.userId)) {
        // Show notification
        setNewExamNotification(`New exam available: ${data.exam.title} in ${data.courseName}`);
        
        // Auto-hide notification after 10 seconds
        setTimeout(() => setNewExamNotification(null), 10000);
        
        // Reload available exams
        loadAvailableExams();
        
        console.log('New exam notification received:', data.exam.title);
      }
    };

    // Register listener
    const removeListener = (window as any).electronAPI?.onNewExamCreated?.(handleNewExam);
  ...
  // Auto-refresh exams every 10 seconds to check for new exams and status changes
  useEffect(() => {
  ...
  // Check for new exams on component mount and show notification
  useEffect(() => {
    let previousExamIds: string[] = [];
      ...
      try {
        if (isElectron()) {
          ...
          if (result.success && result.exams) {
            const currentExamIds = result.exams.map((e: Exam) => e.exam_id);
            
            // Find new exams (not in previous list)
            const newExams = result.exams.filter((exam: Exam) => 
              !previousExamIds.includes(exam.exam_id) && exam.isNew
            );

            // Show notification for new exams
            if (newExams.length > 0 && previousExamIds.length > 0) {
              newExams.forEach((exam: Exam) => {
                setNewExamNotification(
                  `New exam available: ${exam.title} in ${exam.course_name || 'your course'}`
                );
                setTimeout(() => setNewExamNotification(null), 10000);
              });
            }

            previousExamIds = currentExamIds;
          }
        }
      } catch (error) {
        console.error('Error checking for new exams:', error);
      }
    };
    ...
    const interval = setInterval(checkForNewExams, 10000);
  ...
  return (
    <div className="student-dashboard">
      ...
      {newExamNotification && (
        <div className="new-exam-notification">
          <span>🎓 {newExamNotification}</span>
          <button onClick={() => setNewExamNotification(null)} className="close-notification">×</button>
        </div>
      )}
                ...
                {availableExams.length === 0 ? (
                  ...
                  <div className="exam-grid">
                    {availableExams.map((exam) => {
                      ...
                      <div
                        key={exam.exam_id}
                        className={`exam-card ${status} ${isLocked ? 'locked' : ''} clickable`}
                        onClick={() => {
                      ...
                      >
                        <div className="exam-header">
                          ...
                          <span className={`exam-status ${status}`}>
                            {isLocked && 'Locked - Not Started'}
                            {isActive && 'Active - Available Now'}
                            {status === 'ended' && 'Ended'}
                          </span>
                        </div>
                        ...
                        <div className="exam-details">
                          ...
                          {isLocked && (
                            <p className="locked-message">
                              ⏰ This exam will unlock automatically at the start time
                            </p>
                          )}
                          {isActive && (
                        ...
                        <div className="exam-card-footer">
                          <span className="click-hint">
                            {isLocked ? '🔒 Locked until start time' : 'Click to view exam →'}
                          </span>
                        </div>
                      </div>
                    );
