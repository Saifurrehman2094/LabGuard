...
const AdminPanel: React.FC<AdminPanelProps> = ({ currentUser, onLogout }) => {
    ...
    const [activeTab, setActiveTab] = useState<'users' | 'settings' | 'logs' | 'stats' | 'network' | 'exams'>('users');
    ...
    return (
        <div className="admin-dashboard">
            ...
            <div className="dashboard-content">
                                    ...
                                    {auditLogs.length === 0 ? (
                                        ...
                                        auditLogs.map((log, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    {log.action === 'STUDENT_ENROLLED' && log.details ? (
                                                        <div className="enrollment-details">
                                                            <strong>{log.details.studentName}</strong> enrolled in{' '}
                                                            <strong>{log.details.courseName}</strong> ({log.details.courseCode})
                                                        </div>
                ...
                {activeTab === 'network' && (
                    <NetworkSettings currentUser={currentUser} />
                )}

                {activeTab === 'exams' && (
                    <AdminExamViewer currentUser={currentUser} />
                )}
            </div>
