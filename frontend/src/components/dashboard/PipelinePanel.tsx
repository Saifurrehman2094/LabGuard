...
const PipelinePanel: React.FC<PipelinePanelProps> = ({ config }) => {
  ...
  return (
    <div className="dash-panel">
      <div className="dash-panel-header">
        <span className="dash-panel-title">AI Pipeline Config</span>
        <span className={`dash-status-badge ${config.groqConfigured || config.geminiConfigured ? 'dash-status-ok' : 'dash-status-err'}`}>
          {config.groqConfigured || config.geminiConfigured ? 'Active' : 'No Keys'}
        </span>
      </div>
      <div className="dash-pipe-rows">
        {rows.map(r => (
          <div key={r.label} className="dash-pipe-row">
            <span className="dash-pipe-label">{r.label}</span>
            <span className="dash-pipe-value">{r.value}</span>
          </div>
        ))}
      </div>
    </div>
