...
const ConceptHeatmap: React.FC<ConceptHeatmapProps> = ({ concepts }) => {
  ...
  const getBackgroundColor = (failRate: number): string => {
    if (failRate <= 30) return '#dcfce7'; // green
    if (failRate <= 60) return '#fef3c7'; // amber
    if (failRate <= 80) return '#fed7aa'; // orange
    return '#fee2e2'; // red
  };

  const getTextColor = (failRate: number): string => {
    if (failRate <= 30) return '#166534'; // green
    if (failRate <= 60) return '#92400e'; // amber
    if (failRate <= 80) return '#9a3412'; // orange
    return '#991b1b'; // red
  };
        ...
        {concepts.map(concept => (
          ...
          >
            ...
            <div className="card-header">
              {concept.isAtRisk && <span className="at-risk-icon">⚠️</span>}
            ...
            <div className="progress-bar">
              <div
                ...
                style={{
                  width: `${Math.round((concept.passedCount / concept.totalAttempts) * 100)}%`
                }}
              />
              <div
                className="progress-fill fail"
                style={{
                  width: `${Math.round((concept.failedCount / concept.totalAttempts) * 100)}%`
                }}
              />
            </div>
            ...
            <div className="card-meta">
              ...
              <div className="meta-item">
                ...
                <span
                  className="meta-value trend"
                  style={{ color: getTrendColor(concept.trend) }}
                >
                ...
                {concept.attempts.length > 0 && (
                  <div className="expanded-attempts">
                    ...
                    <div className="attempts-list">
                      {concept.attempts.slice(-3).map((attempt, idx) => (
